...
from utils import Logger, AverageMeter, evaluate, Distribution_loss
from dataset.dataset import BUSIDataset_image_roi as BUSIDataset
from networks.prior_fused_model import Prior_fused_model as Teacher
from networks.diagnostic_model import Diagnostic_model
...
def train_test(times):
    config = Config()
    logger = Logger(config.log_path + "diagnostic_model_%s.log"%times)
    ...
    teacher = Teacher(class_num=config.class_num, pretrained=True).cuda()
    teacher.load_state_dict(torch.load(config.model_state_path+"prior_fused_model.pkl"))
    model = Diagnostic_model(class_num=config.class_num, pretrained=True).cuda()

    optimizer = torch.optim.SGD(params=model.parameters(), lr=config.learning_rate, weight_decay=5e-4, momentum=0.9)
    loss_function_diagnosis = nn.CrossEntropyLoss()
    loss_function_distribution = Distribution_loss()
    ...
    for epoch in range(config.EPOCH):
        ...
        for step, (img, mask, label) in enumerate(train_loader):
            ...
            with torch.no_grad():
                features_teacher = teacher(img, mask, distillation=True)
            output, features_student = model(img, distillation=True)
            

            loss_diagnosis = loss_function_diagnosis(output, label)
            loss_distribution = loss_function_distribution(features_student, features_teacher, label)
            loss = loss_diagnosis * 0.8 + loss_distribution * 0.2
        ...
        with torch.no_grad():
            ...
            for step, (img, mask, label) in enumerate(test_loader):
                ...
                label = label.cuda()
                output = model(img)
                loss = loss_function_diagnosis(output, label)
            ...
            if mean > best_mean:
                ...
                torch.save(model.state_dict(), config.model_state_path + "diagnostic_model.pkl")
